...
public class Worker implements Serializable {
	...
	public Worker(String name, ArrayList<Day> days) {
		this.setFields(name, days);
	}

	// Swap 1 - Team 03 - BONUS FEATURE
	//
	public Worker(String name, ArrayList<Day> days, WorkerCalendar cal) {
		this.setFields(name, days);
		this.calendar = cal;
	}
	...
	// The constructor was not modular; therefore, before adding the calendar we
	// needed to extract a method to modularize our constructor usage.

	private void setFields(String name, ArrayList<Day> days) {
		this.name = name;
		this.days = days;
	...
	public boolean checkScheduleConflict(Calendar date){
		return this.calendar.checkScheduleConflict(date);
	}
