...
public class Config extends javax.swing.JFrame {
	...
	public Config(ArrayList<Day> days) {
		...
		/*
		 * Duplicate code was in use for each day of the week. Utilized
		 * reflection and added a method in Day to allow a single block of code
		 * to handle all days.
		 * 
		...
		for (Day day : days) {
			try {
				JCheckBox checkBox = (JCheckBox) this
						.getClass()
						.getDeclaredField(
								day.getNameOfDay().toLowerCase() + "Check")
						.get(this);
				checkBox.doClick();

				ArrayList<String> jobs = day.getJobs();
				for (String job : jobs) {
					this.models[day.getIntOfDay()].addElement(job);
					this.sundayJobList.setModel(this.models[day.getIntOfDay()]);
				}
	...
	private void initComponents() {
		...
		// Swap1 - Team03 QUALITY CHANGE (code duplication)
		/*
		...
		for (int i = 0; i < daysList.length; i++) {
			try {
				Field checkBox = this.getClass().getDeclaredField(
						daysList[i] + "Check");
				final Method checkAction = this.getClass().getDeclaredMethod(
						daysList[i] + "CheckActionPerformed",
						java.awt.event.ActionEvent.class);
				checkAction.setAccessible(true);
				JCheckBox dayCheckBox = (JCheckBox) checkBox.get(this);
				dayCheckBox.setText(daysList[i].substring(0, 1).toUpperCase()
						+ daysList[i].substring(1));
				dayCheckBox.setName(daysList[i] + "Check");
				dayCheckBox
						.addActionListener(new java.awt.event.ActionListener() {
							...
							public void actionPerformed(
								...
								try {
									checkAction.invoke(Config.this, evt);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
		...
		// Swap1 Team03 - Move so it didn't split up the check initialization
